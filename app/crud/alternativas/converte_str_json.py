...
def converte_param_preprocess(preprocessing_params: Dict[str, str]) -> Dict[str, Any]:
    ...
    if 'tamanho_kernel' in preprocessing_params:
        preprocessing_params['tamanho_kernel'] = int(preprocessing_params['tamanho_kernel'])
...
def converter_parametros_para_tipos(segmentation_params: Dict[str, str]) -> Dict[str, Any]:
    # Converte para inteiro
    int_params = [
        'limite_var', 'limite_media', 'referencia_media', 'tamanho_kernel', 
        'iteracoes_morfologia', 'iteracoes_dilatacao', 'tamanho_janela','n'
    ]
    ...
    tuple_params = [
        'lim_hiperaeradas', 'lim_normalmente_aeradas', 'lim_pouco_aeradas', 
        'lim_nao_aeradas', 'lim_osso'
    ]
    for param in tuple_params:
        if param in segmentation_params:
            segmentation_params[param] = tuple(map(int, segmentation_params[param].split(',')))
