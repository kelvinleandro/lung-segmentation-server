...
def calcula_ocorrencias_classes(src: MatLike) -> np.ndarray:
    """
    Calcula a quantidade de ocorrências de cada classe para cada pixel usando operações vetorizadas.
    ...
    for i in range(5):
        ...
        ocorrencias[i] = cv2.boxFilter(mask // 255, -1, (9, 9), normalize=False)[4:-4, 4:-4]
...
def probabilidade_classes(ocorrencias: MatLike) -> np.ndarray:
    ...
    probabilidades[:, mask] = 1./5  # Comportamento original (possível erro lógico)
