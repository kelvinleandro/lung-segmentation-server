...
def aplicar_sauvola(
    ...
    thresh_sauvola = threshold_sauvola(
        imagem_suavizada, window_size=tamanho_janela, k=k
    )
    ...
    if aplicar_morfologia:
        ...
        imagem_segmentada = cv2.morphologyEx(
            imagem_segmentada, cv2.MORPH_CLOSE, kernel, iterations=iteracoes_morfologia
        )
        imagem_segmentada = cv2.morphologyEx(
            imagem_segmentada, cv2.MORPH_OPEN, kernel, iterations=iteracoes_morfologia
        )

    if aplicar_interpolacao:
        imagem_segmentada = cv2.resize(
            imagem_segmentada, None, fx=1.2, fy=1.2, interpolation=cv2.INTER_CUBIC
        )
        imagem_segmentada = cv2.resize(
            imagem_segmentada,
